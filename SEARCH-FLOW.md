...
EXEC-167: 条件の順番で重要度を表す。同じ分類では文章の先に書いた条件ほど重く集計する。結果と履歴の点数内訳で新規検索の条件別「重み」を確認できる。分類間の優先順と旧履歴の保存順位は維持する。

最終確認の「今回の検索」では、商品名の表示枠の直上に「商品名」ラベルを置く。本番接続画面とオフラインモックで同じ配置を使う。


接続画面の商品カードは旧条件説明の折りたたみを削除し、点数の内訳を「検索詳細」という1つの折りたたみで表示する。背景は#212121とし、先頭に英語タイトル（未取得の表示を含む）、その下に日英の点数表と「文章と画像」「画像同士」の独立点を置く。旧履歴は保存時の画像評価・参考合成点を表示する。独立した「Amazonで見る」リンクは置かず、商品名のリンクから商品ページを開く。現在結果と保存済み履歴に同じ表示を使う。接続画面とモックの商品カードでは、商品概要と検索詳細を別の行に配置し、同じカード行内で検索詳細の見出し位置をそろえる。商品名の行数・画像やレビューの有無に左右されず、詳細の開閉でも同じ行の見出しを移動させない。
