...
| 結果読込完了 | 仮表示を件数要約と商品カードへ置換 | 表示件数、検索条件の開閉、実URLがある商品名リンクを有効化 |
| 表示件数を変更 | 表示するカード数だけ即時に増減 | 商品検索や比較は再実行しない |
| `検索条件` を開く | 条件ごとの `一致`・`未確認`・`不一致` をカード内へ展開 | カードごとに開閉し、他のカードや順位を変えない |
...
履歴は新しい順に表示する。各項目には検索日時、商品の種類または自然文の短い要約、価格条件、参考画像利用の有無、比較件数、`YYYY年MM月DD日 HH:mmまで保存` を表示し、同じ行の左に危険ボタンの `削除`、右に `開く` を離して置く。両方の外枠を96×36pxに揃える。
...
`開く` を押すと、保存時点の条件要約と順位付け結果を履歴用の結果画面へ読み込む。画面上部には `保存された検索結果` と検索日時を表示し、価格、在庫、商品ページの内容は現在と異なる可能性があると案内する。`履歴へ戻る` では一覧の同じスクロール位置へ戻る。

履歴を開く、結果を再読込する、表示件数を変える、検索条件を開閉する操作では、条件整理、画像作成、商品取得、情報整理、特徴確認、比較を再実行しない。検索条件は現在結果と同じくカード内で展開する。商品ページは実URLがある場合だけ商品名リンクから開く。
...
参考画像の拡大表示は内容確認だけに使う。条件と価格の編集、`検索語を確認・修正`、待機中の条件要約、商品カードの検索条件、最終確認、結果、履歴一覧は通常のページ内に表示し、Step 3全体、待機画面、結果画面、履歴一覧をモーダルへ入れない。表示件数は選択部品、エラーは発生箇所近くのalertを使う。
