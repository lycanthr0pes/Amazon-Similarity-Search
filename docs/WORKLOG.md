...
## 2026-09-14: 条件の入力順による重み付け（EXEC-167、未コミット）

分類内の開始位置に応じてN..1で平均し、日本語/日英最大値、画像あり/なしへ接続。条件ごとの倍率をschema5履歴と共通点数内訳へ保持する。旧形式は省略fieldと従来平均を維持した。専用20件・関連158件、frontend112件・型/整形/build、実candidateと一時SQLiteのブラウザfixtureで結果/履歴の2倍/1倍を確認。全体3497件成功・既知ctime検査1件失敗。一般ブラウザのクリック/撮影待ちは未解消で、対象の表示検証はキーボード操作を使う。実サービス・稼働server再起動・commit/pushなし。詳細は[EXEC-167](GOAL.md#exec-167-条件を入力順で重み付けする)。

## 2026-09-14: 条件整理中のtimeout表示を削除（未コミット）

利用者指示により、条件整理中の通信timeoutを主画面のalertへ表示しない。状態再取得と接続診断への記録は継続し、他種別のエラー表示と通信timeout自体は維持する。小規模な表示変更として新規testは追加せず、型/整形・既存frontend110件・build成功。配信中のbundleへGETだけで接続し、ブラウザ内の合成timeoutでalertなし・診断保持、その後のnetworkでalert表示・POST0を確認した。実検索やserver再起動は行っていない。

## 2026-09-14: 空行による条件誤判定と位置修正（EXEC-166、未コミット）

末尾の空行が条件fragmentになり、希望/否定の曖昧エラーが局所offsetのまま商品名を指す問題を再現した。parserと視覚条件抽出で空白だけのfragmentを除き、断片のエラー位置を原文位置へ変換した。分類規則・採点・旧履歴は維持する。

RED9件からGREEN、実GiNZA合成回帰を含む関連101件成功。全offline3484件成功・既知ctime検査1件失敗。入力をメモリ内で保持して稼働serverへ反映し、入力/履歴一致と条件整理ボタン有効をGETだけで確認した。モデル推論/検索の自動再実行なし。詳細とhashは [EXEC-166](GOAL.md#exec-166-空行による条件誤判定と修正位置を直す) を参照する。

## 2026-09-14: 接続失敗の種類と診断保持（EXEC-165、未コミット）

状態取得/操作受付の一律エラーを通信・timeout・中断・HTTP・JSON・状態検証・未分類へ分け、固定の検証項目と発生時刻/所要時間を表示。画面の「接続診断」と同一タブのsessionStorageに直近10件を保持し、回復・再読込後も確認可能にした。本文・入力・生例外・stackを保存せず、操作の自動再送は追加していない。

単体RED6件からGREENを確認し、追加境界を含むfrontend110件成功。診断表示のブラウザREDからGREEN、関連6件成功。型/整形/build・Ruff/lock/Markdown/diff成功。稼働serverを再起動せず更新bundle配信と条件確認画面・instance/revision保持を確認した。詳細、hash、描画環境による全E2Eの未確認範囲は [EXEC-165](GOAL.md#exec-165-接続失敗の種類を記録して表示) を参照する。

## 2026-09-14: 条件整理の再起動回復（EXEC-164、未コミット）

同じoutput-dirでの再起動による既存準備directoryとの衝突を、起動別session-IDで解消した。履歴DBの保存先は保持する。状態APIのinstanceIdで画面の更新番号を起動ごとに比較し、前の「整理中」を保持し続けない。新UIは古い起動への確認commandを拒否できるようinstanceIdを送る。

Python/ブラウザの再現REDからGREENを確認。全offline3473成功・既存ctime検査1失敗、frontend型/整形・単体97件・build成功。再起動回復はEnter操作でブラウザ確認し、クリック全回帰は空ページでも再現したChromium描画停止のため未確認。実利用者の停止原因との照合・実モデル推論は未実施。詳細とhashは [EXEC-164](GOAL.md#exec-164-条件整理の再起動後の停止を解消) を参照する。


## 2026-09-13: Bonsaiの起動元終了と同時起動の管理（EXEC-163、未コミット）

共通launcherにLinux/WSLの親終了連動と同一user/portのabstract socket排他を追加。短いisolated Python helperでPR_SET_PDEATHSIGを登録し、特権付きexecを拒否する。ブラウザは競合を固定文で表示する。既存processの自動停止・接続流用や推論retryは追加していない。

合成processによるRED6失敗から同一testのGREEN7成功を確認。追加境界を含む関連99 passed/2 skipped、全offline3466 passed/22 skipped/30 deselected/既存ctime test1 failed。Ruff/format/lock/Markdown/diff成功。実iGPUモデルのhealth-ready後に親を強制終了し、モデル/port/排他解放を12.35秒で確認した。推論/画像生成/商品検索は0回。接続serverへ反映し、入力と履歴一覧一致・idleを確認。詳細なコマンド、hash、制約は [EXEC-163](GOAL.md#exec-163-bonsaiの残存と同時起動による競合を防ぐ) を参照する。
...
## 2026-09-13: Bonsai遅延の調査と実行復旧（EXEC-160）

モデル・プロンプト・厳密な検証を保ってCPU並列度、同一sourceのBLAS版、待機pollを比較した。初回は4スレッドが速かったが再測定では8スレッドが上回り、BLASとpoll変更にも改善根拠がなかった。暫定的な4スレッド固定を含め、コード・テスト・実行ファイル・設定を元へ戻した。

元の非BLAS版で形状・部品・希望/除外の3例の条件保持と形式検証に合格。同一形状要求の8/4スレッド応答と解釈結果はhash一致。元の構成でアプリを再起動し、ブラウザの画像あり条件整理が116.165秒で確認画面へ到達した。形状・価格条件、検索語5件、参考画像生成ボタン、既存履歴1件を確認。追加の画像生成・商品検索は行っていない。

関連51テストと接続fixture26件は成功。全offline回帰は3431 passed/1 failedで、未変更のctime変化を前提にしたテストが単独でも失敗した。全体合格とはしない。現在の動作は復旧したが初回遅延の原因と恒久修正は未確定。比較条件、撤回した変更、実行境界は [EXEC-160](GOAL.md#exec-160-bonsaiの条件整理の遅延を精度を維持して調査) に記録した。


## 2026-09-13: Bonsaiの3ビルド分離とiGPU既定化（EXEC-161）

元のCPU版をbuild-originalへhash一致で保存し、同一llama.cpp sourceからOpenBLAS版build-optimizedとVulkan版build-igpuを構築した。bonsai-runtime.tomlのactiveをigpuに設定し、次の条件整理から選択を読み込む。指定Intel Arc 140Vの事前照合、37層offload、context/モデル維持、親credential非継承を実装。CPUへの暗黙fallbackは設けていない。最適化版の名称は速度向上を保証しない。

WSLの非同期Vulkan試験でdriver待ちの停止が発生したため、同期実行とFP16行列演算経路無効を適用した。最終iGPU設定で37/37層、GPU model buffer 932.68 MiBを確認。実Chromeの画像あり条件整理は87.083秒で確認段階へ到達し、形状・価格、検索語5件、画像生成ボタンと既存履歴1件を確認した。8765番のアプリはiGPU既定で稼働を継続する。

ただし別の連続推論では部品条件が空になる応答があり、CPUとの応答hashも異なる。精度同等性や連続実行の安定性は未確定で、モデル/プロンプト/検証を弱めて成功扱いにはしていない。関連117 passed/2 skipped。同期設定後の全offlineは3441 passed/1 failed/22 skipped/30 deselectedで、失敗は未変更のctime依存テスト。直前構成の全3442 passedとは分けて記録した。今回のモデル実行試験はiGPU版だけで、追加の画像生成・商品検索・課金・commit/pushは行っていない。構築条件、RED/GREEN、診断の失敗を含む詳細は [EXEC-161](GOAL.md#exec-161-bonsaiの3ビルド分離とigpu既定化) を参照する。


## 2026-09-13: 視覚条件の文章対画像を優先する独立採点（EXEC-162）

新規SigLIP 2を文章対商品画像と参考/対比画像対商品画像の2経路へ分離した。確認済み外観記述のtext特徴と既存の商品画像特徴を比較し、条件別最小点を独立保持する。否定・タイトル・優先・希望の後で文章点、画像点、レビューの順に比較する。新しいprofileと任意fieldで旧JSON/SQLite履歴を維持し、旧商品を再採点しない。モデル/プロンプトの追加生成や追加量子化はなく、64-token超過時は切り捨てを拒否する。

モック/本番共通の検索詳細・履歴に「文章と画像」「画像同士」を順に表示し、旧履歴と画像なしは従来表示を保った。Python関連112件、frontend型/整形・Vitest97件・build、モック41件・接続fixture26件・開発15件・追加接続3件は成功。全offlineは3452 passed/1 failed/22 skipped/30 deselectedで、未変更のctime依存テスト1件の失敗が残る。実SigLIP文章workerと合成画像1枚の文章比較をローカルで確認したが、実商品の順位品質の検証とはしない。

8765番を更新して、既存履歴1件と実Chromeの画面読込を確認した。画像生成・商品検索・追加課金・commit/pushは行っていない。RED/GREEN、構築境界、スコア式と検証記録は [EXEC-162](GOAL.md#exec-162-文章対画像と画像対画像の採点分離) を参照する。
