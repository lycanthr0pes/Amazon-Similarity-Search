...
2026-09-14（未コミット、EXEC-167）: 新規検索で同じ分類の条件を入力順に重み付け。先に書いた条件を優先し、結果/履歴の内訳へ倍率を表示。旧履歴の点数・順位は維持する。

2026-09-14（未コミット）: 条件整理中の通信timeoutエラー表示を削除。状態再取得と接続診断の記録は継続する。

2026-09-14（未コミット）: 条件入力の空行を希望/否定の曖昧な条件と誤判定する問題を修正。断片内のエラー位置を原文位置へ変換し、無関係な商品名を修正箇所に表示しない（EXEC-166）。

2026-09-14（未コミット）: 接続エラーの種類と固定の検証項目を表示し、同じタブの「接続診断」に直近10件を保持。回復・再読込後も原因を確認できる。検索入力/応答本文は記録せず、検索の自動再送は行わない（EXEC-165）。

2026-09-14（未コミット）: 接続serverを同じprivate保存先で再起動すると条件整理が既存作業directoryとの衝突で停止する問題と、開いたままの画面が再起動後の更新番号を無視する問題を修正。起動別準備先とinstanceIdを追加し、履歴保存先・採点を維持する（EXEC-164）。

- EXEC-162: 新規SigLIP 2の文章対画像と画像対画像の採点を分離し、文章側を優先。共通モック/本番の結果・履歴に両点を表示し、旧履歴の順位と形式を維持。


EXEC-131: 同一商品の日本語・英語タイトルをPlaywrightで取得し、正規化・履歴保存・接続画面へ通す。英語未取得を区別し、旧履歴と日本語リンクを維持する。CLI cacheはplaywright-v2へ分離する。
...
- EXEC-163: Linux/WSLでアプリ終了時に所有Bonsaiが残る問題を防ぎ、待受開始前の二重起動を排他化。条件整理の競合理由を画面に表示する。

> 状態: 現在の作業変更。未コミット・未マージであり、完了済みreleaseではない。

### Added

- 本番接続画面とオフラインモックの最終確認で、商品名の上に「商品名」ラベルを追加した。

- EXEC-159: 統合済み旧文書・未使用の旧サンプル・対象外デザイン画像43件をローカルの `bin/` へ退避して追跡を解除した。検証用の一時出力も集約し、現行文書は退避物なしで参照できる統合先へ更新した。

- EXEC-158: 履歴の保存案内の重複と余白不足、ラベルが折り返す比較画像カードの位置ずれ、採点表見出しの途中改行を修正した。

- EXEC-157: 既定オフラインモックを現行接続画面と共用化。条件編集・画像選択の再整理・確認・日英採点内訳・履歴・回復操作を同じUIで提供する。固定合成データとタブ内保存を使い、APIへ接続しない。

- 履歴一覧と削除確認の見出しを商品名に変更し、入力文は詳細に保持する。新履歴に任意の商品名を保存し、旧履歴は書き換えずに表示する（EXEC-150）。

- 「参考画像：あり/なし」の状態表示を条件確認・履歴・モックから削除した。画像選択トグルと生成・確認の操作を維持する（EXEC-149）。

- 「指定なし」の編集欄を削除し、原文に許容表現がある場合だけ説明として表示する。ほかの条件を編集しても許容表現を保持する（EXEC-148）。

- 条件確認に優先・希望・否定・指定なしのカンマ区切り編集欄を設け、空欄も編集できるようにした。商品名を条件より上へ移し、末尾の重複フォームを除去。参考画像の「あり/なし」の背景を#212121へ統一した（EXEC-147）。

- 接続画面の不要な案内・折りたたみを削除し、商品名と条件を直接編集できるようにした。「変更を反映」で両方の同義語・英訳を再取得する（EXEC-146）。

- 接続画面の入力から結果・履歴までをモックの共通部品/配置へ統一した。画像トグル、工程別待機、条件へ戻る、確認付き画像作り直し/中止、保存再試行、新しい検索、表示件数/順位/画像拡大/トップへ戻るを接続した。稼働画面へ反映し、旧履歴と待機入力を保持した（EXEC-145）。

- 新しい接続履歴に入力全文・条件名/分類・取得済みの商品サムネイルを保存し、再起動後の履歴画面へ表示する。30日期限と削除に連動し、旧履歴と画像なし検索を維持する（EXEC-144）。

- 接続画面と履歴専用画面へ、確認ダイアログ付きの履歴削除・失敗時再試行・30日期限の自動削除を接続した。履歴と専有画像を同時に削除し、server稼働中は60秒ごとに期限削除を試みる。既存4履歴は期限内で保持し、検索や採点は再実行していない（EXEC-143）。

- 接続画面の「画像なしで進む」を、検索語確認/画像確認/画像準備失敗から最終確認・商品取得・日英採点・SQLite履歴へ接続した。外観条件0件も利用でき、画像用設定・生成・proxy・SigLIPを呼ばない。結果と履歴は画像評価を未使用と表示し、旧結果は保持する。localhost8772へ入力を保持して反映した（EXEC-142）。

- 最新版の接続画面と既存履歴をlocalhost8772へ起動した。初回入力で文章修正が必要になった後、再整理の実行先が未作成で停止する問題を修正した。本番検索は未実行（EXEC-141）。

- 希望・否定・許容表現を共通ローカル解析し、条件確認へ分類と「指定なし」、曖昧な文章の修正箇所を表示する。希望付き予算、否定条件の照合対象分離、日英最大値を接続し、旧計画/履歴を維持する。追加本番検索は行っていない（EXEC-140）。

- 新規candidateの順位を否定条件（負）、タイトル、優先条件、希望条件、画像、レビュースコアの順へ変更した。取得済みの星評価を最終比較へ使い、画面の否定点・レビュー点・履歴の方式識別を接続する。旧履歴は当時の順位を保持する（EXEC-139）。

- 永続SQLiteの履歴一覧・保存結果詳細と検索画面へ戻る操作を接続した。起動時のDB指定と閲覧専用起動に対応し、再読込/サーバー再起動後も商品取得や推論を繰り返さず結果を開ける。旧DBは読み取り専用で保持する（EXEC-138）。

- 接続画面から自然文入力と画像生成前の文章修正を実検索へ渡せるようにした。視覚条件1〜3件に対応し、評価済み画像のサムネイルと日本語・英語・採用点の内訳、独立した画像点を表示する。入力2000文字・再整理2回まで。既存履歴と順位方式を維持する（EXEC-137）。

- OPUS-MT設定時、辞書候補ありからBonsaiが未収録の商品名を提案した場合もローカル英訳へ接続した。Bonsaiへの英訳要求を無効にし、辞書の英訳と翻訳失敗時の日本語候補を保持する（EXEC-134）。

- 条件の同義語・英訳と英語の商品詳細をcandidate採点へ接続し、タイトル・条件ごとの日英最大値と内訳を保存する。画像評価と最終優先順を維持し、新profile/playwright-v3 cacheで旧結果と分離する（EXEC-133）。

- 固定マグカップ入力のReact接続試験入口を追加。ローカルAPIで検索語・参考画像・比較画像・検索の確認段階を管理し、candidate/SigLIP 2経路とSQLite結果表示へ接続する。既定モックを維持し、実サービス実行は個別承認とする。
- ブラウザ接続試験の全体15分制限を撤廃した。検索語/参考画像の確認待ちは無期限とし、個々の通信/推論のtimeout・呼出上限・比較画像生成後の承認期限は維持する。画像承認の期限切れは画面へ表示し、検索の操作を受け付けない。
...
- EXEC-161: Bonsaiの元のCPU版・OpenBLAS版・Vulkan iGPU版を設定ファイルで選択できるようにし、既定をiGPUへ変更。起動前に指定GPUを照合し、モデル・プロンプト・応答検証を維持する。

- EXEC-156: 辞書の対比候補0件をBonsai補完の専用要求として明示。未知外観の一致側/対比側と未知部品名を既存1回の視覚推論で補完し、画像なしの推論省略を維持する。

- EXEC-155: 画像不使用を条件整理のAPIへ渡し、比較画像用推論の失敗で画像なし検索が止まる不具合を修正。外観条件の文章採点を保持し、画像使用への変更は再整理を要求する。

- EXEC-154: 部品付き条件の対比準備で、既存JMdictの英名候補と有無文法を接続。辞書IDまたは部品英名だけのBonsai応答から対比指示をコードで構築する。

- EXEC-153: 対比画像の新規指示を既存WordNetの反対語・文法補助・Bonsai補完へ変更。独自対応表は旧計画の読み取りだけに残し、推論回数と既存履歴を維持する。

- 商品検索・詳細取得をローカルPlaywrightへ移行した。ブラウザ接続・CLI・再実行でOutscraper API keyを要求せず、日本語の商品本文を取得する。新要求/取得元とplaywright-v1 cacheを分離し、旧履歴を維持する。詳細は [EXEC-126](GOAL.md#exec-126-商品取得をplaywrightへ移行)。

- candidateの説明文部分一致を条件順位へ追加し、構造化情報を優先する。元語優先の同義語補助点と、日英カテゴリ・明示ブランド/型番のタイトル比較fieldを追加した。新text profileで旧保存結果と区別し、既存履歴の再採点は行わない。

- Amazon.co.jpの商品リンクを商品IDだけの標準pathと日本語指定へ統一する。API応答と共通商品名リンクへ適用し、保存済み結果も再検索なしで反映する。
...
- 統合元、旧設計、Execution Plan、旧検証資料を [旧文書アーカイブ](REFERENCES.md#統合元文書とアーカイブ) へ集約し、現行正本と履歴資料を分離した
