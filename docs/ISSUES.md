...
2026-09-14: 入力順が条件点へ反映されなかった動作を[EXEC-167](GOAL.md#exec-167-条件を入力順で重み付けする)で変更。分類内の入力順による正規化加重平均と、保存・内訳表示を追加。全offlineの既知ctime検査1件の失敗は残る。一般ブラウザ回帰でクリック前のstable待ちと撮影がtimeoutし未完。今回の採点・保存表示はキーボード操作の対象fixtureで確認。

2026-09-14: 末尾の空白fragmentが曖昧な条件として拒否され、局所offset=0を本文へ適用して先頭の商品名を誤表示する問題を[EXEC-166](GOAL.md#exec-166-空行による条件誤判定と修正位置を直す)で修正。原文を保存/出力しないローカル再現と合成回帰で原因を確認した。


2026-09-14: 状態取得の失敗原因が一律表示で失われていたため、[EXEC-165](GOAL.md#exec-165-接続失敗の種類を記録して表示)で固定コード・検証項目と同一タブ内の直近10件の診断を追加。導入前に発生した障害の種類は遡及特定できない。

2026-09-14: 条件整理が再起動後も進まない経路として、既存作業先との衝突と、画面が新serverの小さいrevisionを捨てる問題を確認。[EXEC-164](GOAL.md#exec-164-条件整理の再起動後の停止を解消)で修正し合成回帰を確認した。利用者の失敗時のエラーは未確認であり、モデル推論の成否とは区別する。
