...
EXEC-139の利用者指示以降、新規candidateの最終順位は否定条件（負）、タイトル一致、優先条件required、希望条件preferred、画像評価、レビュースコアの順。同点時だけ次を比較し、全同点なら取得順。否定条件は一致度が低い方、他は高い方を先にする。レビューは取得済み星評価0〜5、画像/レビュー欠損は比較時に評価済み0より後にする。50:50合成値は参考値とし、旧履歴を新方式で並べ替えない。詳細は [BACKEND.md](BACKEND.md#否定条件優先とレビューの最終比較exec-139) に定義する。
