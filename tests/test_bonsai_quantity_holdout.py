"""Unseen categories use the same frozen judge as the development cases."""
...
from tools.bonsai_quantity_holdout import CASES
...
@pytest.mark.parametrize("case", CASES, ids=lambda c: c.case_id)
def test_local_bonsai_quantity_holdout(pytestconfig, case):
    ...
    try:
        ...
        pytest.fail("Quantity holdout probe failed; inspect private logs", pytrace=False)
    print(json.dumps({"case_id": case.case_id, **result}, sort_keys=True), flush=True)
    if not all(result["checks"].values()):
        pytest.fail("Quantity holdout failed frozen criteria", pytrace=False)
