...
EXEC-156: 辞書に対比候補が0件の条件を既存Bonsai視覚要求へ明示し、未知外観/部品名を補完する。全体offline3386件成功、8772へ反映済み。画像なしではこの推論を呼ばない。実Bonsaiの補完品質は未検証。

EXEC-155: 参考画像オフを条件整理から反映し、比較画像用の推論・対比検証を省く修正を8772へ反映済み。原文条件の日英採点を維持し、画像使用へ変更した場合は再整理する。全体offline3382件・接続ブラウザ23件成功。実モデル/検索/画像生成は追加実行していない。画面再読み込み後の新しい条件整理から有効。

EXEC-154: 部品付き条件の有無文法、既存JMdictの部品英名、辞書ID/未知英名だけのBonsai応答を接続。全体offline3369件成功後に8772へ反映済み。入力と履歴4件を保持した。画面を再読み込みして条件整理を行う。実Bonsaiでの新応答形式と画像品質は未検証。

EXEC-150: 履歴一覧は商品名を表示する。入力文は詳細に保持し、旧履歴を再保存・再採点しない。8772/8771の既存4履歴は静的配信の更新で商品名表示を確認した。


EXEC-149: 「参考画像：あり/なし」の状態表示を削除した。画像選択トグルと既存の検索状態は保持する。


EXEC-148: 「指定なし」の編集欄を削除した。編集対象は優先・希望・否定の3分類。原文の許容表現は説明と内部解釈に保持する。


[EXEC-147](docs/GOAL.md#exec-147-分類ごとのカンマ区切り条件編集): 商品名を条件より上に移し、優先・希望・否定・指定なしをカンマ区切りで編集する。空欄も常時表示し、末尾の重複条件フォームを削除した。参考画像のあり/なしは#212121背景。稼働中検索を維持して画面へ反映した。


[EXEC-146](docs/GOAL.md#exec-146-商品名の直接編集と語彙の再取得): 接続画面の不要な案内と折りたたみを削除し、商品名・条件を直接編集する。「変更を反映」で両方の同義語・英訳を再取得する。稼働中8772の準備済み計画を維持して反映した。追加本番検索なし。


[EXEC-145](docs/GOAL.md#exec-145-オフラインモックと接続画面の全画面操作を統一)で、接続画面の全段階/結果/履歴をモックの共通表示・操作へ揃えた。localhost8772/8771に反映済み。画像ON/OFF、工程別待機、戻って文章修正、確認付き作り直し/安全な中止、保存再試行、新規検索、表示件数/順位/拡大/トップへ戻るを接続した。実状態に基づく操作可否と自然文修正を維持し、モックの固定条件は実画面へ転用しない。既存入力・4履歴保持、追加本番検索なし。以下は各作業時点の記録。

[EXEC-144](docs/GOAL.md#exec-144-元入力条件名商品画像の永続保存)で、元入力全文・条件名/分類・取得済み192px商品PNGの永続履歴への保存/復元を接続し、localhost8772/8771へ反映した。次の新規検索から適用する。画像なし検索は画像を追加取得せず、30日期限/削除へ連動する。既存4履歴の欠落値は補完せず、DBと待機入力を保持している。追加の本番検索/課金なし。以下は各作業時点の記録。

[EXEC-143](docs/GOAL.md#exec-143-履歴削除と期限切れデータ削除の接続)で確認付き履歴削除と30日期限の定期削除を接続した。localhost8772/8771へ反映済み。fixtureでキャンセル・cascade・失敗/応答喪失後の再試行・所有者分離・期限境界を検証し、稼働中は60秒間隔で期限削除を試みる。既存4履歴は期限内のため保持している。以下は過去の各実行時点の記録。

[EXEC-142](docs/GOAL.md#exec-142-画像なし検索を接続)で画像なしの確認・取得・日英採点・結果・永続履歴を接続した。localhost8772は入力を保持して新しいコードへ更新済みで、条件整理から再開する。画像設定/生成/商品画像proxy/SigLIPを呼ばず、画像なしを新profileで保存する。追加本番検索は行わない。以下は各実行時点の記録。

[EXEC-141](docs/GOAL.md#exec-141-新しい条件分類を稼働接続画面へ反映)で最新版の接続画面を http://127.0.0.1:8772/?mode=connected に起動した。条件分類・文章修正と既存4履歴を接続し、初回修正後の実行先作成も修正済み。画面はidleで待機し、今回の本番検索/画像生成/推論は0。旧server/DBは保持した。

[EXEC-140](docs/GOAL.md#exec-140-希望条件と否定条件の自然文指定を拡張)で自然文の希望・否定・指定なし・優先の共通解析、確認表示と文章修正を接続した。希望付き予算の扱いと日英の否定照合を確認し、旧計画/履歴を維持する。今回の確認はoffline/ローカル資材/fixtureブラウザまで。追加本番検索は行わず、既存稼働serverは再起動していない。画像なし接続と未知入力の実運用品質は後続である。

[EXEC-139](docs/GOAL.md#exec-139-否定条件を先頭にしレビュー点を最終比較へ追加)で新規candidateの順位を否定条件（負）＞タイトル＞優先条件＞希望条件＞画像＞レビュースコアへ変更した。同点時だけ次を比較し、レビューは取得済みの5点満点評価を使う。新規起動/検索から適用し、既存server・保存済み履歴は再採点しない。否定点の負表示とレビュー値も画面へ接続した。外部再検索は実施していない。

[EXEC-138](docs/GOAL.md#exec-138-永続履歴の一覧と保存結果を画面へ接続)で永続履歴の一覧・詳細・検索へ戻る操作を接続した。既存4履歴/96商品/生成画像8枚を実ブラウザで確認し、server再起動後も同じ結果を復元した。DBと旧画面は不変、検索/推論/画像生成の追加呼出し0。履歴画面はlocalhost8771。削除/期限切れ物理削除、原入力・条件名・商品画像の永続保存、画像なし・個別条件編集・全画面統一が残る。以下は各実行当時の記録。

[EXEC-137](docs/GOAL.md#exec-137-入力編集と画像採点内訳の画面接続)で、自然文入力（最大2000文字）、画像生成前の文章修正/再整理、視覚条件1〜3件、商品サムネイルと日英点数表を接続した。固定入力の本番フローは222.365秒、24商品の画像・点数・保存/再表示を含む38チェックが成功した。結果はlocalhost8770。初回はメモリ逼迫下で停止し、旧8767/8768/8769を同じURLの軽量閲覧serverへ移して再実行した。画像なし・個別条件カード編集・永続履歴一覧・全画面統一と未知入力/反復運用の品質検証が残る。以下は各実行当時の記録。

[EXEC-135](docs/GOAL.md#exec-135-更新した翻訳日英採点を実接続で確認)で更新した経路の本番接続を完了した。実Bonsaiの名前専用要求＋OPUS-MTの独立確認と、固定マグカップ入力のReact→日英24商品取得→画像評価→採点→SQLite→表示/再読込が成功。全体406.105秒、英語タイトル22件・英語詳細24件。結果はlocalhost8769で閲覧でき、旧8768は保持した。商品名は辞書英訳、条件はローカル翻訳を採用した。任意入力、サムネイル/点数内訳表、条件編集・永続履歴一覧の実UI接続、未知商品の品質検証が残る。

[EXEC-134](docs/GOAL.md#exec-134-商品名提案の英訳をローカル翻訳へ統一)で、OPUS-MT設定時の商品名英訳を辞書優先＋未収録時のローカル翻訳に統一した。辞書候補ありからのBonsai提案も日本語名だけを要求する。視覚条件のBonsaiは引き続き使用し、既存完了結果は再計算しない。実Bonsaiの新要求に対する生成品質は別の検証範囲。

[EXEC-133](docs/GOAL.md#exec-133-条件展開と日英別採点の最大値採用)で、条件のローカル同義語/英訳と英語詳細取得をcandidateへ接続した。タイトル・条件ごとの日英最大値を採用し、画像点と最終順位の優先順を維持する。新規起動した検索から適用し、既存8768の結果は更新しない。CLI cacheはplaywright-v3。任意条件の翻訳品質と反復運用、画面の点数内訳表は引き続き未検証/未実装。

[EXEC-132](docs/GOAL.md#exec-132-条件の辞書ローカル翻訳の単体速度検証)で条件の辞書/翻訳を実資材で単体計測した。10条件の辞書照会中央値0.1832ms、現行翻訳1.9394秒。モデル保持時の参考値0.3695秒は本番未実装。辞書は名詞中心で10条件中6件だけhitした。EXEC-133で接続したが、この単体時間には採点や全同義語の翻訳を含まない。

[EXEC-131](docs/GOAL.md#exec-131-商品の日本語英語タイトルを併せて取得)で日本語/英語の商品タイトル取得・保存・接続画面表示を追加した。匿名の実取得24件で日本語24件、英語22件・未取得2件を確認した。保存/API/Reactはofflineで確認し、英語fieldを含む全体live E2Eは未実施。新規起動・検索から適用し、8768の既存結果は更新していない。CLI cacheはplaywright-v2へ分離する。

[EXEC-130](docs/GOAL.md#exec-130-新優先度で実検索とスコアを再計算)で、新しいタイトル/画像優先順による実再検索を完了した。初回取得停止後、限定診断を経たlive-03が187.560秒で完走。24件の画像評価・各スコア・順序・SQLite保存/React再読込を確認した。予算内22件、不一致2件は17/18位。結果はlocalhost8768、順位別数値はprivate scores.csvで確認できる。初回の停止原因、継続安定性、未知順位品質は未解決/未検証。後段の未実施記録は当時の状態である。

[EXEC-129](docs/GOAL.md#exec-129-ランキングをタイトル画像優先へ変更)で、新規candidateの最終順位をタイトル一致＞画像評価＞否定条件＞優先条件＞希望条件へ変更した。同点時だけ次を比較し、50:50合成点は順位keyに使わない。旧条件優先の保存結果は保持する。新順序は新規起動後の検索から適用し、8767の既存本番結果は実行当時のまま。新順序の実サービス検証は未実施。

[EXEC-128](docs/GOAL.md#exec-128-タイトルと画像の採点比率を等しくする)で新規candidateのタイトル/画像を各50%へ変更した。条件優先と画像欠損時のタイトル点は維持し、新しい比率を結果/履歴へ記録する。既存8767の本番テスト結果は80:20で実行した当時のまま保持し、新比率による実サービス再検索は未実施。

[EXEC-127](docs/GOAL.md#exec-127-playwright移行後の接続本番テスト)で、Playwright移行後の固定入力による本番接続フローを172.081秒で完走した。実Bonsai・画像生成2枚・商品24件・全件SigLIP評価・SQLite保存・React表示/再読込が成功し、日本語タイトル/リンクを全件確認した。予算内22件、超過2件は不一致として末尾に表示する。結果はlocalhost8767で閲覧でき、旧8765は保持した。商品画像の取得/評価は動作するが、接続画面のサムネイル表示は未接続。任意入力・条件編集・モックとの全画面一致・永続履歴一覧と、未知商品/反復運用の品質検証が残る。以下の未検証記録はその実行当時の境界である。

[EXEC-126](docs/GOAL.md#exec-126-商品取得をplaywrightへ移行) で現行の商品取得をPlaywrightへ移行した。検索1ページ→24詳細→正規化の実検証では、全24件の日本語タイトル・円価格・説明・特徴・画像URLを取得した。新規実行にOutscraper API keyは不要で、旧cache/履歴は保持する。継続運用・未知カテゴリ・匿名配送先以外の設定・複数ページの実取得は未検証。後段のOutscraper実行記録は移行前の履歴である。

[EXEC-125](docs/GOAL.md#exec-125-説明文の条件一致とタイトル比較の拡張)で説明文の部分一致・同義語補助・タイトル専用fieldを実装した。新規candidateに適用し、保存済み結果は維持する。実サービスや未知商品の順位品質は未検証で、個別承認後の後続評価とする。

2026-09-11にStreamlitを削除した。Reactの既定モックとCLIを維持し、[EXEC-124](docs/GOAL.md#exec-124-フロントエンドと実検索の接続)で固定マグカップ入力のローカル接続試験入口を追加した。全体15分制限の撤廃後、個別承認と生成画像確認を経たlive-02が完走した。実24商品の取得・画像評価・保存・React表示/再読込を確認済み。次は固定入力以外の画面接続と、別途承認する入力での品質検証。任意入力・画像なし・条件編集・永続履歴一覧の実接続は残る。

## 次期フロントエンドを自前開発する（2026-09-11）

外部納品待ちを解除し、[frontend/](frontend/) にReact + TypeScript + StyleXのデスクトップ画面を実装した。Figmaはレイアウトの参考、色・文字・装飾・操作配置は [視覚仕様](docs/FRONTEND.md#13-次期uiの視覚統一契約) を正本とする。通常ボタンは176×48px、履歴一覧の「削除」「開く」は96×36px、Noto Sans JPと合成SVGはローカル資材を使う。起動手順は [README](README.md#オフラインモック)、検証と残作業の正本は [EXEC-117](docs/GOAL.md#exec-117-react-stylexのオフライン画面) を参照する。

[オフラインモック](docs/FRONTEND.md#14-オフラインモック) は任意入力を確認画面に保持し、固定条件・画像・12件の商品で結果まで進める。画像なし経路・戻る・二重操作防止・指定アニメーション・失敗回復・タブ内履歴を実装し、Chromiumでのオフライン受入確認済みである。実バックエンド・Bonsai・外部API・credentialは使用しない。次の接続作業では、日本語・英語検索語の編集、実APIの表示契約、server側の承認・残数・履歴保存を接続する。`sessionStorage`によるモック履歴を、アプリ再起動後も残る本番履歴の完成とは扱わない。実サービス検証には既存の事前説明と明示承認が必要である。
...
次は実Bonsai・画像生成・実CLIP・候補取得の品質/費用/実行条件を固定して検証する。新flowは視覚条件1〜3件を持つ単一processの注入backendで、既存live入口・API・次期frontendは未切替。視覚条件なしや画像省略での新履歴接続、process再起動後の途中状態復旧は対象外。旧経路の合格率を新経路へ流用しない。次期フロントエンドは自前開発へ移行し、EXEC-117でオフライン画面を実装した。実接続は後続作業とする。
...
- 次期フロントエンドはこのリポジトリで自前開発する。納品待ちを解除し、EXEC-117でオフライン画面を実装した。ローカルAPI接続は後続作業として管理する。
